...
export const editIngredientAsyn = (id, ingredient) => {
    return async (dispatch) => {
        const ingredientRef = doc(db, "risoto", id)
...
export const deleteIngredientAsyn = (id) => {
    return async (dispatch) => {
        await deleteDoc(doc(db, "risoto", id))
        dispatch(listIngredientAsyn())
    }
}
...
export const listIngredientAsyn = () => {
    return async (dispatch) => {
        const query = await getDocs(collection(db, "risoto"))
        const ingretients = []
        query.forEach((doc) => {
            const data = doc.data()
            const DocId = doc.id;
...
export const AddIngredientAsyn = (Ingredient) => {
    return (dispatch) => {
        addDoc(collection(db, "risoto"), Ingredient)
